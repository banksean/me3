...
function getPromptData(document:vscode.TextDocument, position:vscode.Position) {
			// Load document text
...
export function activate(context: vscode.ExtensionContext) {
	...
	const acceptLogger = vscode.window.createOutputChannel('accepted.suggestions', {log:true});
	console.log(`writing accepted suggestions logs: export ACCEPT_LOG=${context.logUri.path}/accepted.suggestions.log`);
	...
	vscode.commands.registerCommand(acceptSuggestCommand, async (...args) => {
		...
		for (const repo of (git?.repositories || [])) {
			if (acceptEvent.fileName.indexOf(repo?.rootUri.path) === 0) {
				...
				acceptLogger.appendLine(JSON.stringify({
					fileName: acceptEvent.fileName.substring(repo?.rootUri.path.length+1),
